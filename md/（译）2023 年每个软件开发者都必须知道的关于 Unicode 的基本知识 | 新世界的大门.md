...
二十年前，[Joel Spolsky 写道](https://www.joelonsoftware.com/2003/10/08/the-absolute-minimum-every-software-developer-absolutely-positively-must-know-about-unicode-and-character-sets-no-excuses/)[1](#user-content-fn-2003)：
...
![](https://tonsky.me/blog/unicode/utf8_trend@2x.png)
...
Unicode 将这些数字称为_码位_（code points）。
...
例如，Unicode 中没有苹果 logo 的位置，因此 Apple 将其放在私用区块中的 `U+F8FF`。在任何其他字体中，它都将呈现为缺失的字形 `􀣺`，但在 macOS 附带的字体中，你就可以看到 。

The Private Use Area is mostly used by icon fonts:

私用区主要由图标字体使用：

![](https://tonsky.me/blog/unicode/nerd_font@2x.png)Isn’t it a beauty? It’s all text! 是不是很漂亮？都是文本哦！
...
<table><thead lang="en"><tr><th>Code point</th><th>Byte 1</th><th>Byte 2</th><th>Byte 3</th><th>Byte 4</th></tr></thead><thead lang="zh"><tr><th>码位</th><th>第 1 字节</th><th>第 2 字节</th><th>第 3 字节</th><th>第 4 字节</th></tr></thead><tbody><tr><td><p>U+<code>0000</code>..<code>007F</code></p></td><td><code>0xxxxxxx</code></td></tr><tr><td><p>U+<code>0080</code>..<code>07FF</code></p></td><td><code>110xxxxx</code></td><td><code>10xxxxxx</code></td></tr><tr><td><p>U+<code>0800</code>..<code>FFFF</code></p></td><td><code>1110xxxx</code></td><td><code>10xxxxxx</code></td><td><code>10xxxxxx</code></td></tr><tr><td><p>U+<code>10000</code>..<code>10FFFF</code></p></td><td><code>11110xxx</code></td><td><code>10xxxxxx</code></td><td><code>10xxxxxx</code></td><td><code>10xxxxxx</code></td></tr></tbody></table>
...
字位（grapheme，或译作字素）[2](#user-content-fn-1) 是在特定书写系统的上下文中最小的可区分的书写单位。`ö` 是一个字位。`é`、`각` 也是。基本上，字位是用户认为是单个字符的东西。
...
Just to be clear: this is NOT correct behavior 让我们先说清楚：这_不是_正确的行为
...
4.  外部，面向人类的 API 一层。UI 中的字数统计。获取前 10 个字符以生成预览。在文本中搜索。像 `.count` 或 `.substring` 这样的方法。Swift 给你_一个视图_，假装字符串是一个字位簇序列。这个视图的行为就像任何人所期望的那样：它为 `"🤦🏼‍♂️".count` 给出 1。
...
2.  C/C++/Java: 使用 [ICU](https://github.com/unicode-org/icu)。它是一个来自 Unicode 自身的库，它对文本分割的所有规则进行编码。
3.  C#: use `TextElementEnumerator`, which is kept up to date with Unicode as far as I can tell.
4.  C#: 使用 `TextElementEnumerator`，据我所知，它与 Unicode 保持最新。
...
12.  自己动手。Unicode [发布](https://www.unicode.org/reports/tr29/#Grapheme_Cluster_Boundaries)了机器可读的规则和表格，上面的所有库都是基于它们的。
...
4.  _或者_还有一个预组合的码位 `U+00C5` 帮你做了这件事。
...
![](https://tonsky.me/blog/unicode/x_variants@2x.png)All of these have their own code points, but they are also all Xs 所有这些字符都有它们自己的码位，但它们也都是 X
...
![](https://upload.wikimedia.org/wikipedia/commons/2/23/Source_Han_Sans_Version_Difference.svg)不同区域设置下的 U+8FD4
...
6.  混起来后再匹配很难。比如保加利亚文中的俄语名字，反之亦然。为什么不呢？这是互联网，来自各种文化的人都在这里冲浪。
...
10.  在需要的时候，我们只能靠猜。例如，Twitter 试图从推文本身的文本中猜测区域设置（因为它还能从哪里得到呢？）时有时会猜错 [3](#user-content-fn-twitter)：

[![](https://tonsky.me/blog/unicode/twitter_locale.jpg)](https://twitter.com/nikitonsky/status/1171115067112398849)

![](https://blog.xinshijiededa.men/tonsky/tweet-translated.webp)
...
![](https://tonsky.me/blog/unicode/english@2x.png)
...
*   数学符号——加号 `+` 和等号 `=` 是 ASCII 的一部分，但减号 `−` 和乘号 `×` 不是 ，
*   various other signs `©` `™` `¶` `†` `§`.
*   各种其他符号 `©` `™` `¶` `†` `§`。

Hell, you can’t even spell `café`, `piñata`, or `naïve` without Unicode. So yes, we are all in it together, even Americans.

见鬼，你甚至不能拼写 `café`、`piñata` 或 `naïve` 而不使用 Unicode。所以是的，我们同舟共济，即使是美国人。

Touché.

法国人：你书的队。[4](#user-content-fn-touche)
...
![](https://tonsky.me/blog/unicode/unicode1@2x.png)Version 1.0 of the Unicode Standard, October 1991Unicode 标准的 1.0 版本, 1991 年 10 月
...
Roadmap to the Unicode BMPThis chart is a roadmap of character allocation for Unicode plane 0: Basic Multilingual Plane (BMP). It was created on 2023-10-15 using Unicode 15.1 data. Characters are categorized more-or-less by the chapters in The Unicode Standard.Лацінская пісьменнасцьНелацінскія еўрапейскія пісьменстваПісьменства АфрыкіПісьменства Сярэдняга Усходу іПаўднёва-Заходняй АзііПісьменства Паўднёвай іЦэнтральнай АзііПісьменства Паўднёва-Усходняй АзііПісьменства Усходняй АзііІдэаграмы ККЯПісьменства Інданезіі і АкіянііПісьменства АмерыкіСістэмы нотапісуЗнакіВобласць для прыватнага выкарыстанняСурагатныя пары UTF-16Па стане на версію Унікода 15.1LatinkaNelatinková evropská písmaAfrická písmaPísma Blízkého a Středního východuPísma jižní a střední AsiePísma jihovýchodní AsieVýchodoasijská písmaČínština, japonština a korejštinaPísma Indonésie a OceánieAmerická písmaNotační systémySymbolyPro soukromé použitíNáhradní páry UTF-16 (surrogate pairs)V Unicode 15.1Lateinische Schriften und SymboleAndere europäische SchriftenAfrikanische SchriftenNahost- und SüdwestasiatischeSchriftenSüd- und MittelasiatischeSchriftenSüdostasiatische SchriftenOstasiatische SchriftenCJK-IdeogrammeIndonesische und ozeanischeSchriftenAmerikanische SchriftenNotationssystemeSymbolePrivater NutzungsbereichUTF-16-SurrogatesStand: Unicode 15.1Latin scriptNon-Latin European scriptsAfrican scriptsMiddle Eastern andSouthwest Asian scriptsSouth andCentral Asian scriptsSoutheast Asian scriptsEast Asian scriptsCJK charactersIndonesian andOceanic scriptsAmerican scriptsNotational systemsSymbolsPrivate useUTF-16 surrogatesAs of Unicode 15.1خط لاتینخط‌های اروپایی غیر لاتینخط‌های آفریقاییخط‌های خاورمیانه و جنوب آسیاخط‌های جنوب آسیا و آسیای میانهخط‌های جنوب شرق آسیاخط‌های آسیای شرقیاندیشه‌نگاری‌های CJKخط‌های اندونزی و اقیانوسیهخط‌های آمریکایینمادگان‌هانمادهاکاربرد شخصیجایگزین‌های UTF-16تا یونی‌کد ۱۵٫۱로마자, 로마자권 기호기타 유럽 문자아프리카 문자중동·서남아시아 문자남부와 중앙 아시아 문자동남아시아 문자동아시아 문자CJK 문자인도네시아, 오세아니아 문자북미 및 남미 문자Notational systems기호사용자 정의 영역UTF-16 상·하위 대체 영역유니 코드 버전 15.1Latijnse schriftenNiet-Latijnse Europese schriftenAfrikaanse schriftenMidden-Oosterse enZuidwest-Aziatische schriftenZuid- en Centraal-Aziatische schriftenZuidoost-Aziatische schriftenOost-Aziatische schriftenCJK-karaktersIndonesische en Oceanische schriftenNoord- en Zuid-Amerikaanse schriftenNotatiesystemenSymbolenPrivégebruikUTF-16-plaatsvervangersGeldig voor Unicode 15.1Латинская письменностьНелатинские европейские письменностиПисьменности АфрикиПисьменности Среднего Востока иЮго-Западной АзииПисьменности Южной иЦентральной АзииПисьменности Юго-Восточной АзииПисьменности Восточной АзииИдеограммы ККЯПисьменности Индонезии и ОкеанииПисьменности АмерикиСистемы нотописиЗнакиОбласть для частного использованияСуррогатные пары UTF-16По состоянию на версию Юникода 15.1Латинська писемністьНелатинські європейські писемностіПисемності АфрикиПисемності Середнього Сходу іПівденно-Західної АзіїПисемності Південної іЦентральної АзіїПисемності Південно-Східної АзіїПисемності Східної АзіїІдеограми ККЯПисемності Індонезії та ОкеаніїПисемності АмерикиСистеми нотописіЗнакиОбласть для приватного використанняСурогатні пари UTF-16Станом на версію Юнікоду 15.1 拉丁文字非拉丁欧洲文字非洲文字中东及西南亚文字南亚及中亚文字东南亚文字东亚文字中日韩汉字印度尼西亚及大洋洲文字美洲文字符号系统符号私人使用区 UTF-16 代理区目前版本为 Unicode 15.1 拉丁文字非拉丁歐洲文字非洲文字中東及西南亞文字南亞及中亞文字東南亞文字東亞文字中日韓漢字印度尼西亞及大洋洲文字美洲文字符號系統符號私人使用區 UTF-16 代理區目前版本為 Unicode 15.1Latin scriptNon-Latin European scriptsAfrican scriptsMiddle Eastern andSouthwest Asian scriptsSouth andCentral Asian scriptsSoutheast Asian scriptsEast Asian scriptsCJK charactersIndonesian andOceanic scriptsAmerican scriptsNotational systemsSymbolsPrivate useUTF-16 surrogatesAs of Unicode 15.1This space on a very crammed Basic Multilingual Plane will never be used for anything good ever again 这个在已经非常拥挤的基本多语言平面上的空间永远不会再被拿来干任何好事
...
一个定长的、涵盖所有人类语言的编码的许诺是如此令人信服，以至于许多系统都迫不及待地采用了它，包括 Microsoft Windows、Objective-C、Java、JavaScript、.NET、Python 2、QT、SMS 和 CD-ROM！
...
2.  有一个能覆盖所有可能语言的编码、
3.  the entire world agrees to use it,
4.  全世界都同意使用它、
5.  we can completely forget about encodings and conversions and all that stuff
6.  我们可以完全忘记编码和转换之类的东西
